...
class Actions(commands.Cog):
    ...
    async def ban(self, interaction: discord.Interaction, member: discord.Member, reason: str = "لم يتم تحديد سبب"):
        # التحقق من الرتب لحماية المشرفين
    ...
    async def warn(self, interaction: discord.Interaction, member: discord.Member, reason: str):
        ...
        # نظام طرد تلقائي إذا وصل العضو إلى 3 تحذيرات
        if total_warns >= 3:
            try:
                ...
                self.user_warnings[member.id] = [] # تصفير التحذيرات له بعد الطرد
            except discord.Forbidden:
                pass

# دالة الربط الأساسية والوحيدة المسموح بها في الـ Cog
async def setup(bot):
